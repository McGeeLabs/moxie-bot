...
import { readRuntimeConfig, readWebhookConfig, readDashboardConfig } from "./core/config";
...
async function shutdown(reason: string, exitCode = 0) {
  ...
  catch { logger.warn("Dashboard shutdown failed"); process.exitCode = 1; }
  try {
...
async function main() {
  ...
  if (!stopping && dashboardConfig) {
    dashboard = new DashboardServer(dashboardConfig, client);
